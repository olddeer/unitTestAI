---
name: implement-test
description: Test implementation agent. Use this agent to write unit tests based on a plan in test-memory-bank/test-tasks.md. Invoke after analyze-test has created the plan and the user is ready to write tests. Runs tests after each implementation, fixes failures immediately, and iterates until 100/100 quality gate is achieved. DO NOT STOP until all tests pass.
...
Refer to `java-test-rule.mdc` for specific good/bad examples and conventions.

### Step 4: Run Tests After Each Implementation
- Run tests via Bash after implementing each test method
...
- Generate coverage report via Bash using the project's coverage tool
